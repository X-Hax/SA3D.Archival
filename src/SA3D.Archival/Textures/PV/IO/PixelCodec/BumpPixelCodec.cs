...
{
	internal class BumpPixelCodec : PVPixelCodec
	{
		public override PVPixelFormat Format => PVPixelFormat.Bump;
		public override int BytesPerPixelPair => 2;
		...
		public override void DecodePixel(ReadOnlySpan<byte> src, Span<byte> dst)
		{
			...
			float azimuth = azimuthCorrect * ((src[0] * n) - 0.5f);
			float elevation = elevationCorrect * (src[1] * n);
		...
		public override void EncodePixel(ReadOnlySpan<byte> src, Span<byte> dst)
		{
			...
			dst[0] = (byte)(azimuth * byte.MaxValue);
			dst[1] = (byte)(elevation * byte.MaxValue);
		}
	}
}
