...
{
	internal class YUV422PixelCodec : PVPixelCodec
	{
		public override PVPixelFormat Format => PVPixelFormat.YUV422;
		public override int BytesPerPixelPair => 4;
		public override int PixelPairSize => 2;

		public override void DecodePixel(ReadOnlySpan<byte> src, Span<byte> dst)
		{
			if(dst.Length > 4)
			{
				sbyte u = (sbyte)(src[0] - 128);
				byte y1 = src[1];
				sbyte v = (sbyte)(src[2] - 128);
				byte y2 = src[3];
