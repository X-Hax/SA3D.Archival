...
{
	internal class DXT1PixelCodec : GVTextureCodec
	{
		private const int _alphaThreshold = 16;

		public override int TransparencyBits => 1;
			...
			for(int yBlock = 0; yBlock < height; yBlock += blockHeight)
			{
				for(int xBlock = 0; xBlock < width; xBlock += blockWidth)
				{
					for(int y = 0; y < blockHeight; y += tileSize)
					{
						...
						{
							RGB565PixelCodec.RGB565ToRGBA8(source[sourceIndex..], colorDst);
							RGB565PixelCodec.RGB565ToRGBA8(source[(sourceIndex + 2)..], colorDst[4..]);

							ushort col1 = (ushort)((source[sourceIndex] << 8) | source[sourceIndex + 1]);
							ushort col2 = (ushort)((source[sourceIndex + 2] << 8) | source[sourceIndex + 3]);
							...
							for(int y2 = 0; y2 < tileSize; y2++)
							{
								for(int x2 = 0; x2 < tileSize; x2++)
								{
									...
									colorDst.Slice(colorIndex, 4).CopyTo(destination[destinationIndex..]);
								}
							}
						...
						for(int x = 0; x < blockWidth; x += tileSize)
						{
							...
							for(int ySubBlock = 0; ySubBlock < tileSize; ySubBlock++)
							{
								for(int xSubBlock = 0; xSubBlock < tileSize; xSubBlock++)
								{
									...
									source.Slice(sourceIndex, 4).CopyTo(subBlock[subBlockStart..]);
									subBlockStart += 4;
								}
							}

							ConvertBlockToQuaterCmpr(subBlock, destination[destinationIndex..]);
							destinationIndex += 8;
						}
					}
				}
		...
		private static void ConvertBlockToQuaterCmpr(ReadOnlySpan<byte> block, Span<byte> destination)
		{
			...
			if(greatestDistance == -1)
			{
				...
				{
					RGB565PixelCodec.RGBA8ToRGB565(block[60..], destination);
					RGB565PixelCodec.RGB565ToRGBA8(destination, paletteDst[4..]);
				}
			}
			else
			{
				...
				RGB565PixelCodec.RGBA8ToRGB565(block[(col1Index * 4)..], destination);
				RGB565PixelCodec.RGB565ToRGBA8(destination, paletteDst);
				ushort col1 = (ushort)((destination[0] << 8) | destination[1]);

				RGB565PixelCodec.RGBA8ToRGB565(block[(col2Index * 4)..], destination);
				RGB565PixelCodec.RGB565ToRGBA8(destination, paletteDst[4..]);
			...
			RGB565PixelCodec.RGBA8ToRGB565(paletteDst, destination);
			RGB565PixelCodec.RGBA8ToRGB565(paletteDst[4..], destination[2..]);
