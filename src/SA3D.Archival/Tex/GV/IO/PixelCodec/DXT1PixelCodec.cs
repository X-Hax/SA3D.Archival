...
{
	internal class DXT1PixelCodec : GVRPixelCodec
	{
			...
			for(int yBlock = 0; yBlock < height; yBlock += blockHeight)
			{
				for(int xBlock = 0; xBlock < width; xBlock += blockWidth)
				{
					for(int y = 0; y < blockHeight; y += tileSize)
					{
						...
						{
							RGB565PixelCodec.RGB565ToRGBA8(src[sourceIndex..], colorDst);
							RGB565PixelCodec.RGB565ToRGBA8(src[(sourceIndex + 2)..], colorDst[4..]);
